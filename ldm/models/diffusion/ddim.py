...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        x_T = None if timesteps is None else self.model.q_sample(z, torch.tensor(self.ddim_timesteps[timesteps-2]).repeat(batch_size).to(z.device)) # 也可能减2
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            ts = torch.full((b,), step, device=device, dtype=torch.long) #batch size 个step
            ...
            # 每一步得到一个去噪后的前一步的latent img, 以及一个DDIM特色的预测的x0
