...
class Adapter(nn.Module):
    def __init__(self, channels=[320, 640, 1280, 1280], nums_rb=3, cin=64, ksize=3, sk=False, use_conv=True):
        ...
        self.unshuffle = nn.PixelUnshuffle(8) # downsample by 8 and expand channels accordingly
        self.channels = channels
        ...
        self.conv_in = nn.Conv2d(cin, channels[0], 3, 1, 1) # cin will be 3 times more, if rgb
        self.cond = nn.Parameter(torch.full((1, 77, 768), 0.), requires_grad=True)
        
    def set_conditioning(self, init_cond):
        self.cond.data = init_cond
    ...
    def forward(self, x):
        ...
        # make a copy of the conditioning vector for each image in the batch
        cond = self.cond.repeat(batch_size, 1, 1)
