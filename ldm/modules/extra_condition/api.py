...
def get_cond_model(opt, cond_type: ExtraCondition):
    if cond_type == ExtraCondition.sketch:
        ...
        return model
    elif cond_type == ExtraCondition.halftone:
        return None
    else:
        raise NotImplementedError
...
def get_cond_halftone(opt, cond_image, cond_inp_type='image', cond_model=None):
    if isinstance(cond_image, str):
        halftone = cv2.imread(cond_image)
        if halftone is None:
            raise RuntimeError("Failed to read cond_image! Path for cond_image might be wrong.")
    else:
        halftone = cv2.cvtColor(cond_image, cv2.COLOR_RGB2BGR)
    halftone = resize_numpy_image(halftone, max_resolution=opt.max_resolution, resize_short_edge=opt.resize_short_edge)
    opt.H, opt.W = halftone.shape[:2]
    halftone = img2tensor(halftone).unsqueeze(0) / 255.
    halftone = halftone.to(opt.device)
    return halftone
...
def get_adapter_feature(inputs, adapters):
    ...
    for input, adapter in zip(inputs, adapters):
        cur_feature, cond = adapter['model'](input)
        cond_weight = adapter['cond_weight']
        if isinstance(cur_feature, list):
            if ret_feat_map is None:
                ret_feat_map = list(map(lambda x: x * cond_weight, cur_feature))
            else:
                ret_feat_map = list(map(lambda x, y: x + y * cond_weight, ret_feat_map, cur_feature))
        else:
            if ret_feat_seq is None:
                ret_feat_seq = cur_feature * cond_weight
            else:
                ret_feat_seq = torch.cat([ret_feat_seq, cur_feature * cond_weight], dim=1)

    return (ret_feat_map, cond), ret_feat_seq
