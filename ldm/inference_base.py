...
def get_base_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='starting point of sampling',
    )
...
def diffusion_inference(opt, model, sampler, adapter_features, append_to_context=None, z=None, timesteps=None):
    adapter_features, cond = adapter_features
    # get text embedding
    c = model.get_learned_conditioning([opt.prompt]) if not opt.learn_prompt else cond
    if opt.learn_prompt:
        print("Using learned prompt")
    else:
        print("Using given prompt")
    if opt.scale != 1.0:
    ...
    samples_latents, _ = sampler.sample(
        S=opt.steps,
        timesteps=timesteps, # from which timestep to sample
        conditioning=c,
        batch_size=1,
        shape=shape,
        verbose=False, # provide more information
        unconditional_guidance_scale=opt.scale,
        unconditional_conditioning=uc,
        z=z, #sample from a specific timestep
