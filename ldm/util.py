...
def load_model_from_config(config, ckpt, vae_ckpt=None, verbose=False):
    ...
    if vae_ckpt is not None and vae_ckpt != 'None':
        ...
        sd = vae_sd["state_dict"] if "state_dict" in vae_sd else vae_sd
...
def load_vae_from_config(config, vae_ckpt, verbose=True):
    print(f"Loading VAE model from {vae_ckpt}")
...
def contrast(img_path):
    ...
    if img_path.endswith('.txt'):
        ...
        for file in image_paths:
            ...
            contrast = (np.max(img) - np.min(img)) / stddev[0][0]
            contrast_list.append(contrast)
            log.write(filename + " " + str(contrast) + "\n")
        log.write("mean: " + str(np.mean(contrast_list)) + "\n")
        return
    if os.path.isdir(img_path):
        print("file is dir")
        log = open(os.path.join(img_path, "_contrast.txt"), "a")
        filelist = os.listdir(img_path)
        contrast_list = []
        for file in filelist:
            if file.startswith("_") or file.find("halftone")!= -1:
                continue
